...
export default function Hero() {
    ...
    const gallery = useMemo(() => {
        const baseImages = content.heroGallery?.length ? content.heroGallery : [content.profile.image];
        const rotatedImages = baseImages.length > 1
            ? [...baseImages.slice(2), ...baseImages.slice(0, 2)]
            : baseImages;

        return {
            col1: [...baseImages, ...baseImages],
            col2: [...rotatedImages, ...rotatedImages],
        };
    }, [content.heroGallery, content.profile.image]);
                ...
                <div className="w-full mt-auto flex flex-col justify-center relative z-20 mix-blend-difference">
                    <div className="overflow-hidden">
                        <h1 className="text-5xl md:text-6xl lg:text-7xl xl:text-8xl 2xl:text-9xl 3xl:text-[140px] font-black tracking-tighter leading-[0.85] text-foreground uppercase whitespace-nowrap">
                            {content.profile.heroTitlePrimary}
                            <br />
                            <span className="text-foreground/80">
                                {content.profile.heroTitleSecondary}
                            </span>
                        </h1>
                    </div>
                </div>
